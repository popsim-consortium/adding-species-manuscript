...
%ILAN: need to get back to this paragraph after I review everything else
This paper is intended to describe the additions to the \stdpopsim catalog,
...
Expanding the species catalog required adding several functionalities to the simulation framework.
We thus upgraded the neutral simulation engine, \texttt{msprime}, from version 0.7.4 to version 1.0. % \citep{Baumdicker2022}.
One of the key features enabled by this upgrade was recombination by gene conversion,
which is essential for modeling genomes of baceria and archea.
...
In bacteria and archaea, homologous recombination occurs primarily through gene conversion rather than single crossover recombinations.
As a result, such species cannot be realistically simulated with a recombination model that considers only crossovers.
In \stdpopsim, gene conversion is simulated by specifying on two parameters: 
the \emph{gene conversion rate}, which is the rate at which a gene conversion event is initiated at a single site;
and \emph{gene conversion tract length}, which is the mean of the geometric distribution from which the length of the transferred segment is sampled.
These parameter values should be specified in the species model in order to enable recombination by gene conversion.
% Ilan: I commented this part out, since it seems a bit redundant
% Similar to species-specific mutation and recombination rates,
% default values for the gene conversion rate and the gene conversion tract length
% can be included in the species catalog, if they pass the quality control step.
% FB: We might want to consider adding some more default gc parameters to the catalog.
% If we do so in time, we should mention that here.
For example, the model for \textit{Escherichia coli} in the \stdpopsim catalog has been updated with a gene conversion rate of $8.9\times 10^{-11}$ and a tract length of 345 bases [ADD CITATION].
We note, however, that in eukaryotes, the gene conversion rate might be considerably higher than the rate of single crossover recombination.
As a result, simulations of eukaryote genomes with gene conversion will typically incur a significant computational load.
Therefore, in \stdpopsim, users are required to explicitly enable gene conversion
by  using the use\_species\_gene\_conversion flag.
% Elise: Not sure the blow fits in the GC paragraph?
% Ilan: the idea is that the model of GC is enabled due to the upgrade to msprime 1.0. I made sure to mention this above. This is still a bit tacked-on here, but still probably the best place to mention this.
In addition to modeling gene conversion, the upgrade of \texttt{msprime} to version 1.0
...
have a small proportion of sites with multiple mutations and possibly more than two alleles \citep{Baumdicker2022}.

Lastly, we extended \stdpopsim to allow augmenting a genome assembly by genome annotations, such as coding regions, promoters, conserved elements, etc.
...
\begin{table}[htb]
	...
	\begin{tabular}{p{1.5in}p{2.2in}p{2.2in}}
			...
			set to 0 or borrow from closest relative with a citeable rate &
			will affect the impact of selection, linkage, and linked selection
			\\
			\hline
			Demographic model &
