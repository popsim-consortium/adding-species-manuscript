...
Methods that use these data for purposes such as inference of demographic history and natural selection
...
To address this, we made use of features of the \texttt{msprime} and \texttt{SLiM} simulation engines for modeling non-crossover recombination \citep{Cury2022}.
...
This update does not affect the rate of crossover recombination, but it adds gene conversion events at a ratio of 83:17 (roughly 5:1) relative to crossover recombination events.
...
There are additional considerations that apply to species with partial genome builds.
We discussed above our choice in \stdpopsim to focus on species with chromosome-level assemblies.
However, simulation is still potentially useful for species with partial genome builds.
...
% Ilan: commented this out, since the level of detail appears to distract some readers
%
% These utilities make it easy to accurately retrieve basic genome information and
% construct the appropriate Python data structures.
...
All resources are available from \href{https://github.com/popsim-consortium/stdpopsim}{https://github.com/popsim-consortium/stdpopsim } 
