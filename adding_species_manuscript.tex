...
Simulation is a key tool in population genetics for both method development and empirical research,
...
and to the more sophisticated inference and simulation software.
However, implementing these simulations still requires substantial time and specialized knowledge.
These challenges are especially pronounced for simulating data for species that were not well-studied,
...
While past method development focused on humans and a few key model systems such as \emph{Drosophila},
...
that are modeled in a given species (or population) of interest.
...
\item
  ...
  should reproduce in simulation to the average observed genetic diversity in that species. Note, however, that this average effective population size will not capture features of genetic variation that are caused by recent changes in population size and the presence of population structure \citep{MacLeod2013,Eldon2015}.
...
it is desireable to choose parameters that best reflect our current understanding of the evolutionary history of the species of interest.
