...
%DIF DEL ../adding_species_manuscript.tex   Thu Sep 22 09:09:24 2022
%DIF ADD adding_species_manuscript.tex      Thu Sep 22 09:11:33 2022
...
However, implementing these simulations \DIFdelbegin \DIFdel{can require }\DIFdelend \DIFaddbegin \DIFadd{still requires }\DIFaddend substantial time and specialized knowledge.
...
The initial version of \stdpopsim \DIFdelbegin \DIFdel{, the species catalog contained information for 
six species, most of which are }\DIFdelend \DIFaddbegin \DIFadd{focused on establishing this framework using 
six }\DIFaddend well-characterized model \DIFdelbegin \DIFdel{organisms}\DIFdelend \DIFaddbegin \DIFadd{species}\DIFaddend .
Here, we report on updates made in the new release of \stdpopsim (version 0.2)\DIFdelbegin \DIFdel{.
In particular,
we describe the }\DIFdelend \DIFaddbegin \DIFadd{,
which include substantial additions to simulation capabilities and a significant expansion of the species catalog.
%DIF > Ilan: moved this up here, to leave the end of the abstract to the bigger ideas
...
Expansion of the species catalog involved }\DIFaddend community-driven \DIFdelbegin \DIFdel{efforts to expand the catalog 
}\DIFdelend \DIFaddbegin \DIFadd{efforts to increase coverage }\DIFaddend more broadly across the tree of life
\DIFdelbegin \DIFdel{, which now contains }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend 21 species, with 25 demographic models and 37 genetic maps.
Our experience \DIFdelbegin \DIFdel{through the community engagement involved in }\DIFdelend \DIFaddbegin \DIFadd{throughout }\DIFaddend this process was that people are indeed keen to put in the time and effort to include their study species,
...
\DIFdelbegin \DIFdel{We also introduce several major advances to the realism of }%DIFDELCMD < \stdpopsim%%%
\DIFdel{'s simulation ability,
including non-crossover recombination and provision of species-specific genomic annotations.
Together, these }\DIFdelend %DIF > Ilan: removed mention of inference methods, which is not the only objective (and not necessarily even the main objective) of the popsim community
\DIFaddbegin \DIFadd{These }\DIFaddend advances to \stdpopsim \DIFdelbegin \DIFdel{will strengthen efforts to use and develop
simulation-based population genomic inference methods,
with particular advances
for }\DIFdelend \DIFaddbegin \DIFadd{aim to further promote the use of realistic whole-genome population genetic simulations,
especially in }\DIFaddend non-model organisms,
...
%DIF <  DRS: I added a citation for one exception I could think of, but it is not especially notable. Any more we could add?
%DIF < Ilan: I think it's sufficient for this point.
%
...
by recombination of short segments \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Didelot2010}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Didelot2010}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
\DIFdelbegin \DIFdel{Since its initial publication in \mbox{%DIFAUXCMD
\cite{Adrion2020}}\hspace{0pt}%DIFAUXCMD
,
...
improved the simulation framework of }%DIFDELCMD < \stdpopsim %%%
\DIFdel{in several ways.
}\DIFdelend %DIF >  Ilan: I think this brief summary is a bit redundant. Better to jump right into the "results"
%DIF >  Since its initial publication in \cite{Adrion2020},
%DIF >  we have increased the number of species in the catalog nearly fourfold,
%DIF >  added multiple demographic models and genetic maps, and
%DIF >  improved the simulation framework of \stdpopsim in several ways.
...
\DIFdelbegin \DIFdel{mostly model }\DIFdelend \DIFaddbegin \DIFadd{thoroughly studied }\DIFaddend species.
However, \DIFdelbegin \DIFdel{these species represent }\DIFdelend \DIFaddbegin \DIFadd{the initial collection of six species represents }\DIFaddend a small slice of the tree of life.
This is a concern
...
\begin{figure}
	...
	\caption{Phylogenetic tree of species available in the \stdpopsim catalog,
		...
		\DIFaddbeginFL \DIFaddFL{Branch length were derived from the divergence times provided by TimeTree5 \mbox{%DIFAUXCMD
\citep{Kumar2022}}\hspace{0pt}%DIFAUXCMD
.
	}\DIFaddendFL \label{fig:tree} }
\end{figure}
...
%DIF > Ilan: applied some important edits in this paragraph to better address some of the delicate situations and options we suggest for them 
That being said, simulation is still \DIFdelbegin \DIFdel{possible and }\DIFdelend potentially useful for species with partial genome builds.
One way to deal with this situation is to \DIFdelbegin \DIFdel{include only }\DIFdelend \DIFaddbegin \DIFadd{treat }\DIFaddend the longer contigs or scaffolds \DIFdelbegin \DIFdel{,
treating them }\DIFdelend \DIFaddbegin \DIFadd{in these builds }\DIFaddend as separate chromosomes in the simulation.
\DIFdelbegin \DIFdel{Some of these contigs will }\DIFdelend %DIF > 
\DIFaddbegin \DIFadd{This approach allows us to model genetic linkage within each contig,
but linkage between different contigs that }\DIFaddend map to the same chromosome \DIFdelbegin \DIFdel{, 
so simulating them separately will not capture the genetic linkage between them.
However, this }\DIFdelend \DIFaddbegin \DIFadd{will not be captured by the simulation.
This }\DIFaddend provides a reasonable approximation for many purposes, at least for genomic regions far from the contig edges.
Short contigs can either be omitted from simulation, or lumped together into one (or several) longer pseudo-chromosome(s).
%DIF < Ilan: I didn't quite get the point about "false precision", so I simplified this statement
\DIFdelbegin \DIFdel{Creating pseudo-chromosomes allows the simulation to fit }\DIFdelend \DIFaddbegin \DIFadd{When lumping short contigs together, one should weigh the benefits of
fitting }\DIFaddend the amount of data of real genomes \DIFdelbegin \DIFdel{, but it artificially increases the }\DIFdelend \DIFaddbegin \DIFadd{and capturing the linkage structure within each contig,
against the adverse effects of inducing false or inaccurate }\DIFaddend correlation between variants \DIFaddbegin \DIFadd{from different contigs.
%DIF > 
In some extreme cases, a sepcies genome build may solely be comprised of very short sequence contigs.
One viable option in such a case is to simulate an anonymous chromosome from which patterns of genetic variation can be extracted (if important, in chunks of size similar to the contigs).
Despite not providing an accurate model for linkage along chromosomes,
this artificial construct does have some benefits,
such as reproducing the main statistical properties of background selection}\DIFaddend .
%
Finally, we note that for some \DIFdelbegin \DIFdel{situations }\DIFdelend \DIFaddbegin \DIFadd{purposes }\DIFaddend it may be sufficient to \DIFdelbegin \DIFdel{rely on simulation of }\DIFdelend \DIFaddbegin \DIFadd{simulate }\DIFaddend a large number of unlinked sites \citep{Gutenkunst2009,Excoffier2013},
which can be generated without any sort of genome assembly.
%DIF <  Ilan: shortened this last part and removed mention of the option to create anonymous chromosomes. IMO, for many purposes, it's probably better to keep the sites independent rather than create aritificial and erroneous linkage between them.
However, this approach would not have the many benefits of whole-chromosome simulations,
which we \DIFdelbegin \DIFdel{discussed in detail earlier}\DIFdelend \DIFaddbegin \DIFadd{promote in }\stdpopsim\DIFaddend .
...
to demonstrate \DIFaddbegin \DIFadd{some of }\DIFaddend the key considerations of the process.

\subsection*{\texorpdfstring{\emph{Anopheles gambiae} (mosquito)}{Anopheles gambiae (mosquito)}}
    \label{AnoGam}

\begin{figure}[b!]
	%DIF < \includegraphics[width=\linewidth]{figs/anoGam_models}
	\DIFdelbeginFL %DIFDELCMD < \centering{\textbf{[FIG TBA]}}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\linewidth]{figs/anogam_demog_table}
	\DIFaddendFL \caption{The species parameters and demographic model used for \emph{Anopheles gambiae} in the \stdpopsim catalog.
	...
	which consists of a single population whose size changes throughout the past 11,260 generations in 67 time intervals. \DIFaddbeginFL \DIFaddFL{The width at each point depicts the effective population size (Ne), with the horizontal bar at the bottom indicating the scale for Ne=$10^6$.
		}\DIFaddendFL \label{fig:anogam} }
\end{figure}
...
%
The more recently updated mutation rate assumed in the species model \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep[\(1.2\times 10^{-8}\) from][as used above]{Harland2017}
}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep[\(1.2\times 10^{-8}\) from][]{Harland2017}
}\hspace{0pt}%DIFAUXCMD
}\DIFaddend is thus \(28\%\) higher than the rate used for inference.
%
...
%
The issue of fitting the rates used in simulation with those assumed during inference was discussed during the independent review of this demographic model, and it raised an important question about recombination rates. Since \cite{MacLeod2013} use runs of homozygosity to infer the demographic model, their results depends on the assumed recombination rate. The recombination rate assumed in inference (\(r=10^{-8}\)) is \(8\%\) higher than the one used in the species model (\(r=9.26\times 10^{-9}\)). In its current version, \stdpopsim does not allow specification of a separate recombination rate for each demographic model, so we had no simple way to adjust for this. Future versions of \stdpopsim will enable such flexibility. Thus, we note that \DIFdelbegin \DIFdel{simulated genomes }\DIFdelend \DIFaddbegin \DIFadd{genomes simulated under the demographic model as currently implemented in }\stdpopsim 
\DIFaddend might have slightly higher linkage disequilibrium than observed in real cattle genomes.
However, we anticipate that this would affect patterns less
than selection due to domestication and selective breeding,
which are \DIFdelbegin \DIFdel{not modeled here}\DIFdelend \DIFaddbegin \DIFadd{as of yet not modeled at all in the simulations of }\stdpopsim\DIFaddend .

\section*{Conclusion}
    \label{conclusion}

As our ability to sequence genomes continues to advance, the need for
population genomic simulation of new model and non-model \DIFdelbegin \DIFdel{organism genomes }\DIFdelend \DIFaddbegin \DIFadd{organisms }\DIFaddend is becoming acute.
So too is the concomitant need for an expandable framework for implementing such simulations \DIFdelbegin \DIFdel{for species of interest and the resources for understanding when and }\DIFdelend \DIFaddbegin \DIFadd{and guidance for }\DIFaddend how to do so.
%DIF < 
\DIFdelbegin \DIFdel{Simulating species of interest, both model and non-model, }\DIFdelend \DIFaddbegin \DIFadd{Generating realistic whole-genome simulations }\DIFaddend presents significant challenges
...
challenges as it provides easy access to \DIFdelbegin \DIFdel{simulations incorporating 
species-specific information, easy inclusion of }\DIFdelend \DIFaddbegin \DIFadd{state-of-the-art simulation engines and practices and an easy procedure for including }\DIFaddend new species genomes\DIFdelbegin \DIFdel{,
and }\DIFdelend \DIFaddbegin \DIFadd{.
Moreover, }\DIFaddend the choices of new species to include are driven by the
needs of the population genomics community. In this manuscript we 
describe the expansion of \stdpopsim in two ways:
the \DIFdelbegin \DIFdel{expansion
of its underlying framework to }\DIFdelend \DIFaddbegin \DIFadd{addition of new features to the simulation framework that }\DIFaddend incorporate new evolutionary processes\DIFaddbegin \DIFadd{,
}\DIFaddend such as non-crossover recombination, which \DIFdelbegin \DIFdel{broadens }\DIFdelend \DIFaddbegin \DIFadd{broaden }\DIFaddend the diversity of species that can
...
We also \DIFdelbegin \DIFdel{present basic considerations }\DIFdelend \DIFaddbegin \DIFadd{compiled a series of guidelines }\DIFaddend for implementing
population genomic simulations, \DIFdelbegin \DIFdel{agnostic to simulation software, }\DIFdelend \DIFaddbegin \DIFadd{which are }\DIFaddend based on
insights from the community-driven process of expanding the \stdpopsim catalog.
We \DIFdelbegin \DIFdel{describe the steps of determining if a species-specific population
genomic simulation is appropriate for the speciesand question, what
data is necessary and why, }\DIFdelend \DIFaddbegin \DIFadd{list the basic requirements for generating a useful chromosome-level simulation for a given species, and explain the rationale behind these requirements.
We discuss }\DIFaddend special considerations for \DIFdelbegin \DIFdel{finding and using
that data,
how to proceed when }\DIFdelend \DIFaddbegin \DIFadd{collecting relevant information from the literature,
and what to do if }\DIFaddend some of that \DIFdelbegin \DIFdel{data }\DIFdelend \DIFaddbegin \DIFadd{information }\DIFaddend is not available\DIFdelbegin \DIFdel{,
and why }\DIFdelend \DIFaddbegin \DIFadd{.
Because this process is quite error-prone,
}\DIFaddend we encourage everyone implementing simulations to have their
parameter choices and implementation reviewed by at least one other
researcher.
These steps can be followed \DIFdelbegin \DIFdel{independently}\DIFdelend \DIFaddbegin \DIFadd{if you are implementing a simulation independently for your own use}\DIFaddend ,
or, as we encourage, \DIFdelbegin \DIFdel{through the }\DIFdelend \DIFaddbegin \DIFadd{when you add your simulation code to }\DIFaddend \stdpopsim\DIFdelbegin \DIFdel{framework for quality control and
to make the species model }\DIFdelend \DIFaddbegin \DIFadd{,
to ensure its robustness and make it }\DIFaddend available for future standardized research.
...
\DIFdelbegin \DIFdel{, with some prior knowledge of mutation and
recombination rates, }\DIFdelend %DIF > , with some prior knowledge of mutation and recombination rates,
will become a candidate for inclusion into the
\stdpopsim catalog following the steps we have outlined above\DIFaddbegin \DIFadd{,
assuming it satisfies our baseline requirements}\DIFaddend .
...
\DIFdelbegin \DIFdel{Moreover, one of the goals of }%DIFDELCMD < \stdpopsim %%%
\DIFdelend \DIFaddbegin \DIFadd{One of the important objectives of the PopSim consortium }\DIFaddend is to leverage \stdpopsim \DIFdelbegin \DIFdel{itself
as a springboard for }\DIFdelend \DIFaddbegin \DIFadd{as means to promote }\DIFaddend education and inclusion of new communities into
computational biology and software development.
We are keen to use outreach, \DIFdelbegin \DIFdel{for instance in the form of }\DIFdelend \DIFaddbegin \DIFadd{such as the }\DIFaddend workshops and hackathons described here, 
as a way to \DIFdelbegin \DIFdel{democratize development of population genomic simulation as well as
}\DIFdelend grow the \stdpopsim catalog and library \DIFdelbegin \DIFdel{generally.
By enabling
researchers of }\DIFdelend \DIFaddbegin \DIFadd{and
democratize the development of population genomic simulations in general.
We predict that the increased use of whole-genome simulations in }\DIFaddend non-model species \DIFdelbegin \DIFdel{with simulation platforms that
}\DIFdelend \DIFaddbegin \DIFadd{will lead to an improvement in inference methods,
which }\DIFaddend traditionally have been quite narrowly focused \DIFdelbegin \DIFdel{with respect to organism}\DIFdelend \DIFaddbegin \DIFadd{on well-studied model organisms.
Thus}\DIFaddend , we hope \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{that further expansion of }\stdpopsim \DIFadd{will }\DIFaddend improve the ease and reproducibility of research across a \DIFdelbegin \DIFdel{large }\DIFdelend \DIFaddbegin \DIFadd{larger }\DIFaddend number of systems,
while simultaneously expanding the community of software developers \DIFdelbegin \DIFdel{at work in the }\DIFdelend \DIFaddbegin \DIFadd{among }\DIFaddend population and evolutionary \DIFdelbegin \DIFdel{genetics world.
%DIF < Ilan: Following suggestions by Dan and Elise, I used something very similar in the abstract now. Will need to think of alternative closing sentences
...
%DIFDELCMD < %%%
%DIF <  Finally, the combination of the quickly increasing amount of whole genome data from
%DIF <  myriad species, and the increases in computational power that have made simulation
%DIF <  of chromosome-length sequences possible, provides a way to increase our confidence
%DIF <  in the realism of these simulations. Whole genome sequences can provide important comparative
%DIF <  data with which to evaluate the simulations. This is a powerful tool for
%DIF <  making sure the simulations correspond to the elements of biological
%DIF <  realism that are important for the intended analyses.
%DIF <  For example, demographic history models inferred with site frequency statistics \citep{Gutenkunst2009}
%DIF <  can be validated by comparing inferences from haplotypes (generated from MSMC, for example)
%DIF <  in simulated and real data \citep[e.g.,][]{Hsieh2016a}.
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  ILAN: moved this here from first results section. We might want to expand as a discussion point.
%DIF < 
%DIF < However the advantage working together to validate population genomic
%DIF < model implementation is not restricted to species that are added to
%DIF < the \stdpopsim catalog. Researchers implementing population genomic
%DIF < simulations are encouraged to have their code and parameter choices
%DIF < checked by at least one other person before using it to create
%DIF < simulations. In addition, basic genomic features of the simulation
%DIF < results (e.g. the site frequency spectrum, the extent of linkage disequilibrium, etc.)
%DIF < can be compared to the reference genome and/or known population
%DIF < genetic characteristics. In this too it is useful to have the input
%DIF < of another researcher with fresh eyes.
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  ILAN: moved this here from first results section. We might want to expand as a discussion point.
%DIF < 
%DIF < Finally, what about \emph{whole genome} simulations? Chromosomes
%DIF < segregate independently, so between-chromosome correlations are generally close
%DIF < to zero. But they can be occur in fairly extreme situations, such as intense
%DIF < directional or stabilising selection on multiple loci across chromosomes
%DIF < \citep{Bulmer1971, Lara2022}. However, this situation can be simulated in
%DIF < follow-up forward-in-time simulations \citep{Haller2018, Gaynor2020}. For
%DIF < this reason, we tend to simulate chromosomes independently, and few
%DIF < simulators have mechanisms to simulate
%DIF < multiple chromosomes simultaneously.
%DIF <  DRS: SLiM kind of lets you do this...
%DIF <  ILAN: I decided to remove this for now.
%DIF <  MEL: I don't think it's necessary in this paper anymore.
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  ILAN: moved this here from second results section. We might want to expand as a discussion point.
%DIF < 
%DIF <  However not all species models that were started
%DIF <  at the hackathon were added, as we learned that there is a disconnect
%DIF <  between what species community members wish to simulate, and those
%DIF <  species that have sufficient resources for a realistic simulation.
%DIF < 
%DIF <  When we set out to cast a wide net and add a wide variety of species to
%DIF <  the catalog, we quickly ran into species that people were enthusiastic
%DIF <  to add, but lacked many (or most) of the parameters estimates discussed above. The
%DIF <  utility of \stdpopsim is to make complex, multifaceted population genomic models
%DIF <  easily available for simulation; such data includes genetic maps, annotations, and/or
%DIF <  demographic models. We have not yet encountered a species with
%DIF <  widely-used demographic models but no chromosome-level assembly, so the
%DIF <  main issue in practice seems to be around chromosome-level assemblies
%DIF <  and around matching genome parameters to demographic models.
%DIF < 
%DIF <  However, there is no clear line for what level of assembly quality is
%DIF <  required to be ``useful'' - the most telling indication is whether there is
%DIF <  a community of users eager to use it.
%DIF < 
\DIFdelend \DIFaddbegin \DIFadd{geneticists.
}\DIFaddend 
